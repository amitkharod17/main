...
function AdminNewBatch({
  ...
  const [assignedStudId, setAssignedStudId] = useState([]);
  const [discussionData, setDiscussionData] = useState({});
  const [isDiscussionForumCreated, setIsDiscussionForumCreated] =
    useState(false);
  ...
  useEffect(() => {
    if (toBeEditedBatch?._id) {
      ...
      setIsForumEnabled(toBeEditedBatch.discussionForum[0]?.isActive);
      ...
      axios.get(`/api/course/enroll/all`, config).then((res) => {
        ...
        res.data.forEach((estud) => {
          if (estud.batchData && estud.batchId !== null) {
            if (estud.batchData._id === toBeEditedBatch._id) {
              ...
              _assignedStudId.push(estud._id);
        ...
        setAssignedInstructors(assignedInstruct);
        setAssignedStudId(_assignedStudId);
        //console.log(assignedStud);
        //console.log(_eCourse[0]);
        //console.log(assignedInstruct);
        //console.log(_assignedStudId);
      });
    }
  ...
  const addNewBatch = async () => {
    ...
    if (toBeEditedBatch?._id) {
      ...
      const body = {
        ...
        school,
        discussionForum: discussionData,
  ...
  const handleStudentSearchChangeRemove = (ai) => {
    ...
    if (toBeEditedBatch?._id) {
      ...
      if (window.confirm(`Are you sure to assign ${name} from this batch`)) {
        ...
        setAssignedStudents(allAssignedStudents);
        axios
          .delete(`/api/course/enroll/deleteuser/${_sid}`, config)
          .then((res) => {
            //console.log(res.data);
            //console.log(allAssignedStudents);
            setAssignedStudents(allAssignedStudents);
  ...
  const handleForum = async () => {
    ...
    axios
      .post(`/api/discussion/${toBeEditedBatch._id}`, config)
      .then((res) => {
        console.log(res.data);
        setDiscussionData(res.data);
        setIsDiscussionForumCreated(true);
      })
      .catch((e) => console.log(e));

    // if (isDiscussionForumCreated) {
    //   const body = {
    //     discussionForum: discussionData,
    //   };
    //   const dataBody = [body];
    //   axios
    //     .put(`/api/course/batch/${toBeEditedBatch._id}`, dataBody, config)
    //     .then((res) => console.log(res.data))
    //     .catch((err) => console.log(err));
    // }
  };

  const handleDiscussionVisibility = async (id) => {
    const userInfo = localStorage.getItem("userInfo");
    const token = userInfo ? JSON.parse(userInfo).token : "";
    const config = {
      headers: {
    ...
    console.log(id);
    const body = {
      isActive: !isForumEnabled,
    };
    console.log(body);
    if (id) {
      axios
        .put(`/api/batch/discussion/${toBeEditedBatch._id}/${id}`, body, config)
        .then((res) => {
          console.log(res.data);
          alert(isForumEnabled ? "Forum disabled" : "Forum Enabled");
        })
        .catch((err) => console.log(err));
    }
  };
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/discussion/${toBeEditedBatch?._id}`, config)
      .then((res) => {
        return setDiscussionData(res.data);
              ...
              <>
                <div className="adminNewBatch__inputSection">
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleDiscussionVisibility(discussionData[1]?._id)
                    }
                  >
                    {isForumEnabled ? "Disable" : "Enable"}
                  </button>
                </div>
              </>
            )}
